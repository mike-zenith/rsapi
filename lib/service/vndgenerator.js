...
var extend = require('node.extend'),
    util = require('util'),
    ormHelper = require('./vndgenerator/orm');
...
vnd.prototype.applyItems = function (items, skeleton) {
    var fillValues = false,
        isPrimaryKey = function (item) {
            return item.name === '_id' || item.name === 'id';
        },
        generateHref = function (p, id, i, arr) {
    ...
    skeleton.data = this.orm.toTemplate(items, fillValues);
    skeleton.href = skeleton.data.filter(isPrimaryKey).reduce(generateHref, skeleton.href);
