...
/*
generator.prototype.store = function (key, config, mountpath) {
    var Rest = require(process.cwd() + '/lib/controller/restful');

    config.forEach(function (cfg) {
        var key = cfg.verb + ' ' + mountpath;
        key += mountpath.substr(-1) === '/' && cfg.uri[0] === '/' ?
            cfg.uri.substr(1) :
            cfg.uri;
        Rest.controllers[key] = cfg.handler;
    });
};
*/
